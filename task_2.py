...
def rod_cutting_table(length: int, prices: List[int]) -> Dict:
    ...
    for n in range(1, length + 1):
        ...
        for i in range(n):
            if prices[i] + dp[n - (i + 1)] > max_profit:
                max_profit = prices[i] + dp[n - (i + 1)]
                best_cut = [i + 1] + cut_solution[n - (i + 1)]
        
        dp[n] = max_profit
        cut_solution[n] = best_cut
    
    return {"max_profit": dp[length], "cuts": cut_solution[length], "number_of_cuts": len(cut_solution[length]) - 1}
